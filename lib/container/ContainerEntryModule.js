/*
	...
	Author Tobias Koppers @sokra, Zackary Jackson @ScriptedAlchemy, Marais Rossouw @maraisr, Jacob Ebey @jacob-ebey
*/
...
module.exports = class ContainerEntryModule extends Module {
	...
	codeGeneration({ moduleGraph, chunkGraph, runtimeTemplate }) {
		...
		for (const block of this.blocks) {
			...
			const chunk = chunkGroup.chunks.filter(
				chunk => !chunk.hasRuntime() && chunk.id !== null
			)[0];

			modules.push(
				`${JSON.stringify(name)}: ["${chunk.id}", "${mod.id}"]`
			)
		...
		const source = Template.asString([
			...
			Template.indent([
				...
				`exposedModules: ${runtimeTemplate.returningFunction("exposedModules")}`
			]),
			"});"
		]);
